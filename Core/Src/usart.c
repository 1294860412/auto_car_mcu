...
void send_uart7_data(uint8_t* data, uint8_t len) 
{
    
    //HAL_UART_Transmit(&huart7, packet, len + 5, 1000);
		//HAL_UART_Transmit_DMA(&huart7,uattbuffdma,strlen(uattbuffdma));//send string
		//HAL_UART_Transmit_DMA(&huart7, packet, len + 5);//DMA
}

void process_packet()
{
    // process the received packet
	HAL_UART_Transmit(&huart7, (uint8_t*)rxBuffer, rx_len, 200);
  memset(rxBuffer, 0, RX_BUFFERSIZE);
	rx_len=0;
	recv_end_flag=0;
}
...
void HAL_UART_IDLECallback(UART_HandleTypeDef *huart)
{
    if (huart->Instance == UART7)
    {
        ...
        uint32_t dma_cndtr = __HAL_DMA_GET_COUNTER(huart->hdmarx);
        // 计算接收到的数据长度
        rx_len = RX_BUFFERSIZE - dma_cndtr;
        recv_end_flag=1;
			        // 重新启动DMA接收
        HAL_UART_Receive_DMA(&huart7,rxBuffer,RX_BUFFERSIZE);//重新打开DMA接收 
    }
}
/* USER CODE END 1 */
